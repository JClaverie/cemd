...
def _build_atomic_system_from_coords(
    positions: np.ndarray, types: list[str], box: np.ndarray = None
) -> AtomicSystem:
    """
...
def build_surfaces(
    ...
    refine_slabs: bool = False,  # False by default to keep the original
    ...
    ----------
    data : AtomicSystem
        The initial structure. Should have '_pmg_struct' for best results.
    miller_indices : Sequence[int]
    ...
    refine_slabs : bool, default=False
        If True, refine the generated slabs using SpacegroupAnalyzer.
        Set to False to preserve the original slab structure.
    ...
    # ===============================================================================
    # 1. Recover the ORIGINAL structure (maximum priority)
    # ===============================================================================
    struct = None
    source = None

    # 1a. Try to use the original unrefined structure
    if hasattr(data, "_pmg_struct"):
        struct = data._pmg_struct
        source = "_pmg_struct (original)"
        logger.info("Using original (unrefined) structure for surface generation.")

    # 1b. Otherwise try _pmg_struct (can be refined)
    elif hasattr(data, "_pmg_struct"):
        struct = data._pmg_struct
        source = "_pmg_struct (possibly refined)"
        logger.warning(
            "Using '_pmg_struct' which may be refined. "
            "Surface generation works best with the original structure. "
            "Consider using '_pmg_struct' instead."
        )

    # 1c. Fallback: convertir AtomicSystem -> pymatgen
    else:
        try:
            struct = data.to_pmg()
            source = "to_pmg() conversion"
            logger.info("Using converted structure from AtomicSystem.to_pmg().")
        except Exception as e:
            raise RuntimeError(
                f"Cannot generate surfaces: {e}\n"
                "The AtomicSystem does not have '_pmg_struct' or '_pmg_struct', "
                "and to_pmg() failed."
            ) from e

    # ===============================================================================
    # 2. Check that the structure is valid
    # ===============================================================================
    if not isinstance(struct, Structure):
        raise TypeError(f"Expected pymatgen Structure, got {type(struct).__name__}.")
    ...
    if not struct.is_ordered:
        logger.warning("Structure is not fully ordered. Surface generation may fail.")
    ...
    # ===============================================================================
    # 3. Add oxidation states (if possible)
    # ===============================================================================
    try:
    ...
    # ===============================================================================
    # 5. Create the surface generator
    # ===============================================================================
    try:
    ...
    # ===============================================================================
    # 6. Find the minimum number of broken links
    # ===============================================================================
    slabs_list_pmg = []
    actual_broken = -1

    # Try with bindings first
    ...
    for i, raw_slab in enumerate(slabs_list_pmg):
        try:
            # If we ask for refinement, apply it
            if refine_slabs:
                try:
                    analyzer = SpacegroupAnalyzer(raw_slab)
                    slab = analyzer.get_refined_structure()
                    logger.debug(f"Slab {i} refined successfully.")
                except Exception as e:
                    logger.warning(f"Could not refine slab {i}: {e}")
                    slab = raw_slab
            else:
                slab = raw_slab

            # Convert to AtomicSystem
            # IMPORTANT: refine=False to not re-refine
            as_system = AtomicSystem.from_pmg(slab, refine=False)

            # KEEP the original structure of the slab
            # to be able to generate other surfaces if necessary
            if not hasattr(as_system, "_pmg_struct"):
                as_system._pmg_struct = slab

            # Also keep the original structure of the parent
            if hasattr(data, "_pmg_struct"):
                as_system._parent_structure = data._pmg_struct

            slabs_list_as.append(as_system)

        except Exception as e:
            logger.error(f"Failed to convert slab {i} to AtomicSystem: {e}")
            continue
...
def explore_surfaces(data: AtomicSystem) -> AtomicSystem | None:
    ...
    miller_str = questionary.text(
        "Enter Miller indices (e.g. 1 0 0):", default="1 0 0"
    ).ask()
    if not miller_str:
        return None
    ...
    slabs, shifts, dipoles, n_broken = build_surfaces(
        data, miller, min_slab_size=float(min_slab), min_vacuum_size=float(min_vac)
    )
    ...
    def render():
        header = HEADER_FORMAT.format(
            cursor=" ", idx="#", shift="Shift", dipole="Dipole (D)", natoms="N atoms"
        )
        lines = [
    ...
    # Same pattern as explore_pubchem
    selected_idx = [0]
    kb = KeyBindings()
...
def build_solution(
    box: Sequence[float] | np.ndarray = [30, 30, 30],
    density: float = 1.0,
    ...
    if solutes_dict:
        for key, num in solutes_dict.items():
            if structures_dict and key in structures_dict:
                ...
                raise ValueError(
                    f"The structure '{key}' is neither a known ion nor an open tab."
                )
        ...
        if os.path.exists(pdb_molecule):
            ...
            try:
                ...
                for element, amount in breakdown.items():
                    ...
                    atomic_composition[element] = atomic_composition.get(element, 0) + (
                        amount * coeff
                    )
                ...
                raise ValueError(
                    f"Component '{formula}' not found (no PDB or valid formula)"
                )
            ...
            for key, num in solutes_dict.items():
                if structures_dict and key in structures_dict:
                    struct_path = os.path.join(
                        tmp, f"custom_{id(structures_dict[key])}.pdb"
                    )
                    structures_dict[key].write(struct_path)
                else:
                    struct_path = get_structure_path(key, tmp)
...
def split(
    ...
    if add_solution:
        ...
        liquid_data = build_solution(
            [vec_list[0][0], vec_list[1][1], gap_size],
            density,
            solutes_dict,
            structures_dict,
        )
...
def merge(
    ...
    if lmp_data_a.velocities is not None and lmp_data_b.velocities is not None:
        merged_velocities_df = np.concatenate(
            [lmp_data_a.velocities, lmp_data_b.velocities]
        )
    ...
    for key in connectivity_keys:
        if b_connectivity_dic[key] is not None:
            b_connectivity_dic[key].iloc[:, 1:] = (
                b_connectivity_dic[key].iloc[:, 1:].astype(int) + lmp_data_a.num_atoms
            )
        else:
    ...
    for key in connectivity_keys:
        df_merged = pd.concat(
            [a_connectivity_dic[key], b_connectivity_dic[key]], ignore_index=True
        )
