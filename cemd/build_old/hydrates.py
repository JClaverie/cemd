...
from ._interlayer_helpers import distribute_species_in_layers, fill_csh_interlayers
...
def pycsh(
    ...
    -------
    list[AtomicSystem]
        A list of C-S-H atomic systems.
    """
    ...
    try:
        ...
        for line in process.stdout:
            ...
            if match and progress_callback:
                ...
                msg_status = "pyCSH is running, it could take some time..."
    ...
    for i in range(1, nsamples + 1):
        ...
        try:
            ...
            raise RuntimeError(f"Error reading generated file {reax_name}: {e!s}")
    ...
    return (
        atomic_systems_list[0] if len(atomic_systems_list) == 1 else atomic_systems_list
    )
...
def build_csh(
    ...
    nsi_to_remove, nca_to_add, vacancy_fraction = calculate_csh_modifiers(
        nsi, nca, cs_ratio, min_mcl
    )

    if progress_callback:
        progress_callback(10, "Randomly removing brindging silicates...")
    ...
    num_water = (
        final_system.get_count("Hw")
        + final_system.get_count("Hh")
        + final_system.get_count("H")
    ) / 2
...
def csh_to_cash(
    atomic_system: AtomicSystem, as_ratio: float, supercell_factor: int = 1
) -> tuple[AtomicSystem, float]:
...
def build_af(ws_ratio: float, supercell: Sequence[int] = None) -> AtomicSystem:
