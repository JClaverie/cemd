...
def init_user_forcefields_database(force: bool = False) -> None:
    ...
    if copied > 0:
        print(f"Created user force field directory: {user_dir}")
        print(f"Copied {copied} TOML files.")
        print(f"Edit these files to customize force field parameters.")
    else:
        print(f"All force field files already exist in: {user_dir}")
        print(f"To overwrite, use: init_user_forcefields_database(force=True)")
