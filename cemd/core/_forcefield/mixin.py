...
"""
ForceFieldMixin for AtomicSystem.
"""
...
from ..._paths import FF_DATABASE_DIR
from .database import ForceFieldDatabase
from .models import (
...
class ForceFieldMixin:
    ...
    def set_ff_from_database(self, 
        ...
        if ff_database_dir is None:
            ff_database_dir = FF_DATABASE_DIR
        
        db = ForceFieldDatabase(ff_database_dir)
        
        # ✅ Résoudre les assignments en types complets (format explicite)
        resolved_assignments = {}
        for sys_type, db_type in assignments.items():
            # Si déjà un full_type, on le garde
            if '.' in db_type:
                resolved_assignments[sys_type] = db_type
            else:
                # Chercher le type dans tous les modèles
                found = None
    ...
    def explore_ff_database(self, ff_database_dir: str = None) -> None:
        """
        ...
        ff_database_dir : str, optional
            ...
            Defaults to FF_DATABASE_DIR (global constant).
        ...
        for el, t in zip(system_elements, system_types):
            ...
            if selected:
                # ✅ Utiliser le type complet pour l'assignation
    ...
    def _validate_and_convert_coeffs(self, coeffs, expected_len, name):
        """Valide et convertit une liste de coefficients."""
    ...
    def set_angle_param(self: AtomicSystem, 
                            angle_type: str, 
                            coeffs: list[float]) -> None:
        """
    ...
    def set_dihedral_params(self: AtomicSystem, 
        ...
        # Pour les dièdres, on ne valide pas le nombre de coeffs car ça dépend du style
        ...
        self.dihedral_params[dihedral_type] = coeffs  # Ou une dataclass si définie
    ...
    def apply_pair_mixing_rules(self, rule='arithmetic', overwrite=False) -> None:
        ...
        if missing_self_params:
            raise ValueError(
                ...
                "You must set these parameters with 'set_ff_pair_param' before blending."
            )
        ...
        for t1, t2 in combinations(self.atom_types, 2):
            ...
            if key_cross not in self.pair_params or overwrite:
                ...
                # Stocker le résultat comme LJParams
