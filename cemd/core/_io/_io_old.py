...
class IOMixin:
    ...
    def from_file(cls, path: str) -> Self:
        ...
        if ext == '.data':
            ...
            topology = cls._read_pmg(refined_struct, raw_struct.lattice.abc)
    ...
    def from_dict(cls, topo_dict: dict) -> Self:
        ...
        ----------
        topo_dict
            Dictionary containing the topology information (atoms, box, etc.).
    ...
    def from_smiles(cls, smiles: str) -> Self:
        ...
        ----------
        smiles
            String representing the molecule in SMILES format.
    ...
    def from_mda(cls, obj: mda.Universe | mda.AtomGroup) -> Self:
        ...
        ----------
        obj
            MDAnalysis Universe or AtomGroup object.
    ...
    def write(self: AtomicSystem, fout: str, atom_style: str='full', oldstyle: bool=False) -> None:
        ...
        if ext == '.pdb':
            _write_pdb(self, fout)
        else:
            _write_lmp(self, fout, atom_style, oldstyle)

    def to_mda(self: AtomicSystem) -> mda.Universe:
        ...
        univ.add_TopologyAttr('name', self.atoms.type.to_numpy())
        univ.add_TopologyAttr('charge', self.atoms.charge.to_numpy())
        univ.add_TopologyAttr('ids', self.atoms.index)
        univ.atoms.positions = self.atoms[['x','y','z']].to_numpy()
        univ.dimensions = self.box

        univ_masses_dic = {t:m for t, m in zip(self.atom_types, self.masses)}
        masses = []
        for atype in self.atoms.type:
            masses.append(univ_masses_dic[atype])
        univ.add_TopologyAttr('masses', masses)

        if self.bonds is not None:
            bonds = list(zip(
                self.bonds['atom_1'].astype(int) - 1,
                self.bonds['atom_2'].astype(int) - 1))
            univ.add_TopologyAttr('bonds', bonds)

        if self.angles is not None:
            angles = list(zip(
                self.angles['atom_1'].astype(int) - 1,
                self.angles['atom_2'].astype(int) - 1,
                self.angles['atom_3'].astype(int) - 1))
            univ.add_TopologyAttr('angles', angles)

        if self.dihedrals is not None:
            dihedrals = list(zip(
                self.dihedrals['atom_1'].astype(int) - 1,
                self.dihedrals['atom_2'].astype(int) - 1,
                self.dihedrals['atom_3'].astype(int) - 1,
                self.dihedrals['atom_4'].astype(int) - 1))
            univ.add_TopologyAttr('dihedrals', dihedrals)

        if self.impropers is not None:
            impropers = list(zip(
                self.impropers['atom_1'].astype(int) - 1,
                self.impropers['atom_2'].astype(int) - 1,
                self.impropers['atom_3'].astype(int) - 1,
                self.impropers['atom_4'].astype(int) - 1))
            univ.add_TopologyAttr('impropers', impropers)

        if self.velocities is not None:
            univ.trajectory.ts.velocities = self.velocities
    ...
    def to_pmg(topology: AtomicSystem | dict) -> Structure:
        ...
        if hasattr(topology, 'atoms'):
            ...
            # assume that the object has a method to retrieve [a, b, c, alpha, beta, gamma]
            # Otherwise we use topology.box
            box_params = topology.box 
        else:
            df_atoms = topology['atoms']
            # If lmp_box (xhi, xlo...), must convert it back to [a, b, c, alpha, beta, gamma]
        ...
        # ensure that the types are indeed character strings (eg: 'C', 'H')
            ...
            if not type_label and types_in_coeffs:
                ...
                if key == "Pair Coeffs":
                    ...
                    for row in values:
                        ...
                        pair_params_raw[type_id] = coeffs
                ...
                if key == "Bond Coeffs":
                    ...
                    for row in values:
                        type_id = int(row[0])
                        coeffs = [float(x) for x in row[1:]]
                        bond_params_raw[type_id] = coeffs
                ...
                if key == "Angle Coeffs":
                    ...
                    for row in values:
                        type_id = int(row[0])
                        coeffs = [float(x) for x in row[1:]]
                        angle_params_raw[type_id] = coeffs
                ...
                if key == "Dihedral Coeffs":
                    ...
                    for row in values:
                        type_id = int(row[0])
                        coeffs = [float(x) for x in row[1:]]
                        dihedral_params_raw[type_id] = coeffs
                ...
                if key == "Improper Coeffs":
                    ...
                    for row in values:
                        type_id = int(row[0])
                        coeffs = [float(x) for x in row[1:]]
                        improper_params_raw[type_id] = coeffs

                if 'pair_params_raw' in locals() and atom_types:
                    for type_id, coeffs in pair_params_raw.items():
                        label = str(atom_types[type_id - 1])  # type_id commence à 1
                        pair_params[label] = coeffs
                
                if 'bond_params_raw' in locals() and 'bond_types' in locals():
                    for type_id, coeffs in bond_params_raw.items():
                        label = str(bond_types[type_id - 1])
                        bond_params[label] = coeffs

                if 'angle_params_raw' in locals() and 'angle_types' in locals():
                    for type_id, coeffs in angle_params_raw.items():
                        label = str(angle_types[type_id - 1])
                        angle_params[label] = coeffs

                if 'dihedral_params_raw' in locals() and 'dihedral_types' in locals():
                    for type_id, coeffs in dihedral_params_raw.items():
                        label = str(dihedral_types[type_id - 1])
                        dihedral_params[label] = coeffs

                if 'improper_params_raw' in locals() and 'improper_types' in locals():
                    for type_id, coeffs in improper_params_raw.items():
                        label = str(improper_types[type_id - 1])
                        improper_params[label] = coeffs
    ...
    @staticmethod
    def _read_mda(mda_universe: mda.Universe) -> dict:
    ...
    @staticmethod
    def _read_pmg(pmg_structure: Structure, 
    ...
    def _read_sdf(source: str) -> dict:
        ...
        ----------
        source
            Path to the SDF file or raw SDF string content.

        Returns
        -------
        dict
            Topology dictionary containing positions, atom types, and the calculated box.
        """
...
def read_log(log_file: str) -> list[pd.DataFrame]:
    """Extracts thermodynamic data from a LAMMPS log file.
    ...
    -------
    list[pd.DataFrame]
        A list of pandas DataFrames, where each DataFrame corresponds to a calculation phase ('run').
    """
...
def _write_lmp(lmp_data: AtomicSystem, 
              fout: str, 
              atom_style: str, 
...
def _write_pdb(self, fout: str) -> None:
        """
