...
class TCLGenerator:
    ...
    def material(material: str) -> str:
        ...
        lines.extend(f"material change {key} {material} {value}" 
    ...
    @classmethod
    def generate_representations(cls, material: str, resolution: int,
                                 atom_types: Set[str] = None) -> str:
        """Generate full representations TCL."""
        if material not in VMD_MATERIAL_OPTIONS:
            raise ValueError(f"Unknown VMD material '{material}'. "
                           f"Available: {VMD_MATERIAL_OPTIONS}")
        print("\n".join([
...
def view(
    ...
    with TempFileManager() as tmp:
        ...
        config_file = tmp.create_file("vmd_config.tcl", TCLGenerator.generate_config(system))
        rep_file = tmp.create_file("vmd_rep.tcl", 
