...
def reshape_crystal(crystal, water_in_crystal, shape):
    ...
    crystal_rs = [
        [[0 for k in range(shape[2])] for j in range(shape[1])] for i in range(shape[0])
    ]
...
def get_angles(crystal_dict, water_dict, shape):
    ...
    for cell in crystal_dict.keys():
            ...
            for i_O in range(len(ind_O)):
                for j_O in range(i_O + 1, len(ind_O)):
                    angle_entries.append(
                        [angle_index, 2, ind_O[i_O], ind_Si, ind_O[j_O]]
                    )
                    angle_index += 1
        ...
        if "<Ro" in crystal_dict[cell] or "<R" in crystal_dict[cell]:
            ...
            if next_cell[1] == shape[1]:
                next_cell[1] = 0
            next_cell = tuple(next_cell)

            next_piece = "<Lo"
            if "<L" in crystal_dict[next_cell]:
                next_piece = "<L"
        ...
        if ">Lo" in crystal_dict[cell] or ">L" in crystal_dict[cell]:
            ...
            if prev_cell[1] == -1:
                prev_cell[1] = shape[1] - 1
            prev_cell = tuple(prev_cell)

            prev_piece = ">Ro"
            if ">R" in crystal_dict[prev_cell]:
                prev_piece = ">R"
        ...
        for w in water_dict[cell]:
            angle_entries.append(
                [
                    angle_index,
                    1,
                    water_dict[cell][w][1][0],
                    water_dict[cell][w][0][0],
                    water_dict[cell][w][2][0],
                ]
            )
            angle_index += 1
...
def transform_surface_separation(
    crystal_entries, supercell, unitcell, surface_separation
):
...
def check_move_water_hydrogens(crystal_entries):
    ...
    for itry in range(10):
        ...
        for iwater in range(len(list_Ow)):
            ...
            for i in range(100):
                # print(iwater, i)
                ok_molecule = check_new_molecule(
                    iwater,
                    list_Hw,
                    list_Ow,
                    list_oH,
...
def list_distance_Ow(
    list_Hw, list_Ow, list_oH, list_O, aux_entries, min_dist_H=0.8, min_dist_O=1.0
):
    N_water = len(list_Ow)


def check_new_molecule(
    iwater,
    list_Hw,
    list_Ow,
    list_oH,
    ...
    for iH in range(len(list_oH)):
        ...
        aux = np.concatenate(
            (
                aux,
                [
                    np.array(aux_entries[list_oH[iH]][3:])
                    - np.array(aux_entries[iHw][3:])
                ],
            )
        )
    ...
    for iw in range(2 * (iwater - 1)):
        ...
        aux = np.concatenate(
            (
                aux,
                [
                    np.array(aux_entries[list_Hw[iw]][3:])
                    - np.array(aux_entries[iHw][3:])
                ],
            )
        )
    ...
    for iO in range(len(list_O)):
        ...
        aux = np.concatenate(
            (
                aux,
                [
                    np.array(aux_entries[list_O[iO]][3:])
                    - np.array(aux_entries[iHw][3:])
                ],
            )
        )
    ...
    for iO in range(len(list_Ow)):
        ...
        aux = np.concatenate(
            (
                aux,
                [
                    np.array(aux_entries[list_Ow[iO]][3:])
                    - np.array(aux_entries[iHw][3:])
                ],
            )
        )
...
def check_new_molecule_old(
    ...
    for iH in range(len(list_oH)):
        iHw = 2 * iwater
        iHw = list_Hw[iHw]
        dist = check_distance_PBC_old(
            np.array(aux_entries[list_oH[iH]][3:]), np.array(aux_entries[iHw][3:])
        )
        if dist < min_dist_H:
            return False
        iHw = 2 * iwater + 1
    ...
    for iw in range(2 * (iwater - 1)):
        ...
        iHw = list_Hw[iHw]
        dist = check_distance_PBC_old(
            np.array(aux_entries[list_Hw[iw]][3:]), np.array(aux_entries[iHw][3:])
        )
        if dist < min_dist_H:
            return False
        iHw = 2 * iwater + 1
    ...
    for iO in range(len(list_O)):
        ...
        iHw = list_Hw[iHw]
        dist = check_distance_PBC_old(
            np.array(aux_entries[list_O[iO]][3:]), np.array(aux_entries[iHw][3:])
        )
        if dist < min_dist_O:
            return False
    ...
    for iO in range(len(list_Ow)):
        ...
        iHw = list_Hw[iHw]
        dist = check_distance_PBC_old(
            np.array(aux_entries[list_Ow[iO]][3:]), np.array(aux_entries[iHw][3:])
        )
        if dist < min_dist_O:
            return False
