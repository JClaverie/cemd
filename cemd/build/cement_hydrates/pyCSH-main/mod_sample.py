...
def sample_Ca_Si_ratio(
    sorted_bricks, Ca_Si_ratio, W_Si_ratio, N_brick, widths, offset=[0.0, 0.0]
):
    ...
    while True:
        while len(crystal) != N_brick:
            ...
            if (
                abs(key_Q - u2) < 1.0
                and abs(key_SiOH - u3) < 0.3
                and abs(key_CaOH - u4) < 0.3
            ):
                ind = np.random.randint(
                    len(sorted_bricks[key_Ca_Si][key_Q][key_SiOH][key_CaOH])
                )
                ...
                N_SiOH += sorted_bricks[key_Ca_Si][key_Q][key_SiOH][key_CaOH][
                    ind
                ].N_SiOH
                N_Oh += sorted_bricks[key_Ca_Si][key_Q][key_SiOH][key_CaOH][ind].N_Oh

                N_braket += sorted_bricks[key_Ca_Si][key_Q][key_SiOH][key_CaOH][
                    ind
                ].N_braket
        ...
        if brick_Q == 0:
            list_elegible_water = np.array(
                [len(crystal[i_brick].elegible_water) for i_brick in range(N_brick)]
            )
...
def fill_water(crystal, N_water):
    ...
    for i_brick in range(N_brick):
        aux = np.random.choice(
            crystal[i_brick].elegible_water, size=water_distr[i_brick], replace=False
        )
