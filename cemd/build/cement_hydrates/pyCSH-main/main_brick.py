...
if create:
    ...
    offset = [0.0, 0.0]
    if offset_gaussian:
        off_Si, off_Ca = get_offset(
            500, sorted_bricks, Ca_Si_ratio, W_Si_ratio, N_brick, widths
        )
    ...
    for isample in range(N_samples):
        ...
        if new:
            list_properties.append(
                [N_Ca / N_Si, r_SiOH, r_CaOH, MCL, jsample + 1, r_2H_Si]
            )

            water_in_crystal = fill_water(crystal, N_water=N_water)

            crystal_rs, water_in_crystal_rs = reshape_crystal(
                crystal, water_in_crystal, shape
            )
            entries_crystal, entries_bonds, crystal_dict, water_dict = (
                get_full_coordinates(crystal_rs, water_in_crystal_rs, shape, pieces)
            )

            entries_angle = get_angles(crystal_dict, water_dict, shape)

            # Water molecule overlap
            entries_crystal, N_not_ok, itry = check_move_water_hydrogens(
                entries_crystal
            )

            if N_not_ok != 0:
                print(
                    f"Warning: structure {jsample: 5d} contains {N_not_ok: 5d} wrong water molecules"
                )
            else:
                print(f"Structure {jsample: 5d} converged after {itry: 5d} iterations")
    ...
    print(f"Generation completed in {time.time() - t: 12.6f} s")
...
if read_structure:
    shape, crystal_rs, water_in_crystal_rs, N_Si, N_Ca, r_SiOH, r_CaOH, MCL = (
        read_brick(shape_read, brick_code, water_code, pieces, surface_from_bulk)
    )
    ...
    if surface_separation:
        entries_crystal, supercell = transform_surface_separation(
            entries_crystal, supercell, unitcell, surface_separation
        )
