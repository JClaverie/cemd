...
def get_offset(N_samples, sorted_bricks, Ca_Si_ratio, W_Si_ratio, N_brick, widths):
    ...
    for isample in range(N_samples):
        crystal, N_Ca, N_Si, r_SiOH, r_CaOH, MCL, _, _ = sample_Ca_Si_ratio(
    ...
    return exp_SiOH(Ca_Si_ratio) - np.mean(aux_SiOH), exp_CaOH(Ca_Si_ratio) - np.mean(
        aux_CaOH
    )
...
def check_SiOH_CaOH_MCL(sorted_bricks, widths, shape):
    ...
    if compute_offset:
        for Ca_Si_ratio in list_target_ratios:
            off_Si, off_Ca = get_offset(
                N_samples, sorted_bricks, Ca_Si_ratio, W_Si_ratio, N_brick, widths
            )
    ...
    with open("check_exp.dat", "w") as f:
        for i in range(len(list_target_ratios)):
            ...
            f.write(
                fmt.format(*list_CaSi[i], *list_SiOH[i], *list_CaOH[i], *list_MCL[i])
            )
