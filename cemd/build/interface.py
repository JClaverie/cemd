...
from .._utils import lattice2vectors, vectors2lattice, lattice2lammps
...
def _calculate_liquid_box(system: AtomicSystem, thickness: float, axis: str) -> list[float]:
...
def _build_droplet_from_blueprint(blueprint: SolutionBuilder,
                                  radius: float,
...
def _merge_data(system_a: AtomicSystem,
                system_b: AtomicSystem,
                box: np.ndarray) -> AtomicSystem:
    """
    ...
    for key in connectivity_keys:
        if b_connectivity[key] is not None:
            ...
            b_connectivity[key].iloc[:, 1:] = b_connectivity[key].iloc[:, 1:].astype(int) + system_a.num_atoms
        else:
    ...
    # Set new topology
    for key in connectivity_keys:
        df_merged = pd.concat([a_connectivity[key], b_connectivity[key]], ignore_index=True)
...
def _merge_structure(base_system: AtomicSystem,
                    structure_to_add: AtomicSystem,
                    distance: float,
                    axis: str = 'z',
                    vacuum: float = 0.0) -> AtomicSystem:
    """
...
def add_liquid_layer(system: AtomicSystem,
...
def add_droplet(system: AtomicSystem,
                blueprint: SolutionBuilder,
                radius: float,
                distance: float = 2.0,
                vacuum: float = 10.0,
...
def add_vacuum(system: AtomicSystem,
               thickness: float,
               axis: str = 'z') -> AtomicSystem:
    """
