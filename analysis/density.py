...
def density_profile(universe: mda.Universe, 
    ...
    """Create a csv file with the average density of atoms of given types along a given axis.
    ...
    ----------
        ...
        types
            Atom types
        axis
            Axis along which to calculate the density
        start
            Starting frame
        end
            Ending frame
        delta
            Size of bins for the histogram (in angström)
...
def density_map(univ: mda.Universe, 
              ...
              bin_size: float = 0.1) -> pd.DataFrame:
    """Create a csv files with the average density of atoms of given types between to coordinates along a given axis.
...
def find_interfaces_coordinates(input_df: pd.DataFrame,
    ...
    """Calculate the solid/liquid interface coordinates for a interfacial system.
...
def shift_profile(input_df: pd.DataFrame, 
    ...
    ----------
        input_df
            Input DataFrame of 1D atomic density profiles
        shift
            Positive or negative shift in angstrom
        csv_out
            Output CSV file
...
def electrostatic_potential(input_df: pd.DataFrame, 
    ...
    """Calculate the charge distribution, electric field and electrostatic potential from a 1D densities csv file.
    ...
    ----------
        ...
        list_charges: list
            List of the charges for each atom types in the CSV file
